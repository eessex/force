...
module.exports = class FollowButtonView extends Backbone.View
  ...
  onToggle: (e) ->
    ...
    if @isLoggedIn
      ...
      location.href = "/sign_up?action=follow&objectId=#{@followId}&kind=artist&redirect-to=#{window.location}&intent=follow+#{@type.toLowerCase()}&trigger=click&contextModule=#{@context_module}"
